...
public class ControleFuncionario extends HttpServlet {
	...
	protected void excluir(HttpServletRequest request,
		...
		try {
			...
			FuncionarioDao fd = new FuncionarioDao();
			//Criando um funcionário, que esta recebendo o funcionario buscado
			Funcionario f = fd.buscarPorId(id);
			if(f == null) {
				request.setAttribute("msg", "<div class='alert alert-danger'>O funcionário não existe</div>");
			}else {
				if(fd.excluir(id)) {
					request.setAttribute("msg", "<div class='alert alert-danger'>O funcionário foi excluido</div>");
				}else {
					request.setAttribute("msg", "<div class='alert alert-danger'>O funcionário não foi excluido, tente novamente</div>");
				}
			}
		}catch (Exception e) {
			e.printStackTrace();
